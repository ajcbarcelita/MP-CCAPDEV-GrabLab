...
export function useProfile() {
  ...
  const filteredReservations = computed(() => {
    ...
    if (selectedBuilding.value === 'All') {
      ...
      return activeReservations.filter(reservation =>
        reservation.lab_slot?.lab?.building === selectedBuilding.value
      )
    }
  })
  ...
  async function deleteAllUserReservations(userId) {
    ...
    const userReservations = reservationsStore.reservations.filter(res => res.user && res.user.user_id === userId)
  ...
  async function handleChangePicture(event) {
    ...
    if (!file) return
    try {
      const updatedUser = await usersStore.updateUserProfilePicture(currentUser.value.user_id, file)
  ...
  if (confirm('Are you sure you want to cancel this reservation?')) {
    try {
      ...
      const index = reservationsStore.reservations.findIndex(r => r._id === reservationId)
      if (index !== -1) {
