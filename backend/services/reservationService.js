  ...
  async createReservationWithSession(reservationData, req) {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        const { user_id, lab_id, reservation_date, slots, anonymous } = reservationData;
        ...
        for (const slot of slots) {
          ...
          if (conflict) {
            throw new Error(`Seat ${slot.seat_number} at ${slot.start_time}-${slot.end_time} is already reserved`);
          }
        }
  ...
  async updateReservationWithSession(reservationId, updateData, req) {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        const existingReservation = await Reservation.findById(reservationId).session(session);
  ...
  async deleteReservationWithSession(reservationId, req) {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        const deletedReservation = await Reservation.findByIdAndDelete(reservationId, { session });
