/**
 * User Model
 * Represents a user of the system (Student or Technician).
 * - user_id: Unique numeric ID
 * - email: DLSU email address
 * - password: Hashed password
 * - fname/lname/mname: Name fields
 * - role: Student or Technician
 * - status: Active/Inactive
 * - profile_pic_path: Path to profile picture
 * - description: User bio/description
 *
 * See backend/controllers/userController.js for how users are registered, authenticated, and managed.
 */
...
const userSchema = new mongoose.Schema(
  {
    ...
    email: {
      ...
      trim: true,
      match: [
        /^[a-zA-Z0-9._%+-]+@dlsu\.edu\.ph$/,
        "Please enter a valid DLSU email address.",
      ],
    },
    password: {
    ...
    role: {
      type: String,
      required: true,
      enum: ["Student", "Technician", "Admin"], // Added Admin role
      default: "Student",
    },
    status: {
      type: String,
